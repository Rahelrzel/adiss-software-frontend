...
import type { Song } from "../types/playlist.type";
...
const createSong = async (data: CreateSongParams, token: string) => {
  const song = await axios
    .post<Song>(`${apiUrl}/api/songs`, data, authHeader(token))
    .then((res) => res.data);
  return song;
};
...
const getSongs = async (token: string) => {
  const songs = await axios
    .get<Song[]>(`${apiUrl}/api/songs`, authHeader(token))
    .then((res) => res.data);
  return songs;
};
...
const getSongById = async (id: string, token: string) => {
  const song = await axios
    .get<Song>(`${apiUrl}/api/songs/${id}`, authHeader(token))
    .then((res) => res.data);
  return song;
};
...
const updateSong = async (data: UpdateSongParams, token: string) => {
  const song = await axios
    .put<Song>(`${apiUrl}/api/songs/${data.id}`, data, authHeader(token))
    .then((res) => res.data);
  return song;
};
